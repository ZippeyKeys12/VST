...
- Intros i.
  ...
  Exists (i+1); entailer!.
  assert (i <> Zlength ls) by cstring.
  rewrite (sublist_split 0 i) by rep_lia. rewrite Forall_app.
  list_solve.
Qed.
...
- fastforward.
  ...
  {
    fastforward.
    replace (andb _ _) with false by cstring.
    finish.
  }
  zsolve.
  assert (HZnth: Byte.signed (Znth i (ls1 ++ [Byte.zero])) =
     Byte.signed (Znth i (ls2 ++ [Byte.zero]))) by lia.
  finish.
  (* Finished transaction in 7.036 secs (6.958u,0.043s) (successful) *)
Qed.
